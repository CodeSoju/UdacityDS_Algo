...
callsFromBangaloreToBanglore = 0
...
def answerNumber(phoneNumber):
  if phoneNumber[0] == '(':
    return phoneNumber.split(')', 1)[0][1:]
  else:
...
for call in calls:
  if isBangalore(call[0]):
    callsFromBangalore += 1
    result = answerNumber(call[1])
    codesList.add(result)
    
    if isBangalore(result):
      callsFromBangaloreToBanglore += 1

#Part A
codesList = sorted(codesList)
print("The numbers called by people in Bangalore have codes:")
for code in codesList:
  print(code)

#Part B
fraction = float(callsFromBangaloreToBanglore) / float((callsFromBangalore))
